...
export default class FolderBackup {
    ...
    async makeBackups(count = 3, type) {
        ...
        if (!this.fm.exists(pathToBackups))
            await this.fm.makeDir(pathToBackups)
        ...
        let tmpBackupDir = path.normalize(tempDirectory + '/' + randomString.generate())
        let tmpArchiveDir = path.normalize(tempDirectory + '/' + randomString.generate());
